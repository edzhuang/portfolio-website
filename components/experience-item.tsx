...
export function ExperienceItem({ years, organization, title, description, className = "" }: ExperienceItemProps) {
  return (
    <div className={`grid grid-cols-4 gap-12 ${className}`}>
      <div className="flex items-center col-span-1 text-body-md text-muted-foreground h-12">{years}</div>
      <div className="col-span-2 flex flex-col gap-2">
        <h3 className="flex items-center text-heading-md text-foreground h-12">{organization}</h3>
